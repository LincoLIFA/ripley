...
export class TaskboardNGRXComponent implements OnInit, OnDestroy {
  ...
  addTask() {
    ...
    modalRef.result.then((result) => {
      ...
      const newTask = this.task(result.title, result.message, result.type);
  ...
  editTask(task: Task) {
    ...
    modalRef.result.then((result) => {
      ...
      const newTaskObj = this.createObject(task);
  ...
  updateTaskStatus(id: string, status: string, task?: Task) {
    ...
    const newTaskObj = this.createObject(currentTask);

    newTaskObj.status = status;
    newTaskObj.badgeClass = badgeClass;

    const index = this.tasks.indexOf(this.tasks.find(x => x.taskId === currentTask.taskId));
